...
const MapContainer = (props) => {
  const [customPlaces, setCustomPlaces] = useState([
    { lat: 28.6139, lng: 77.2090, name: 'Delhi', state: 'Delhi', district: 'Central Delhi', link: 'http://192.168.1.203:3001' },
    { lat: 19.0760, lng: 72.8777, name: 'Mumbai', state: 'Maharashtra', district: 'Mumbai City', link: '#' },
    { lat: 16.7050, lng: 74.2433, name: 'Kolhapur', state: 'Maharashtra', district: 'Kolhapur', link: '#' },
    { lat: 12.9716, lng: 77.5946, name: 'Bengaluru', state: 'Karnataka', district: 'Bengaluru Urban', link: '#' },
  ]);
  ...
  const handleStateChange = (event) => {
    ...
    setSelectedDistrict(''); // Reset selected district when the state changes
  };
  ...
  const districtsByState = {
    ...
    'Andhra Pradesh': [
      'Anantapur', 'Chittoor', 'East Godavari', 'Guntur', 'Krishna', 'Kurnool', 'Prakasam', 'Sri Potti Sriramulu Nellore', 'Srikakulam', 'Visakhapatnam', 'Vizianagaram', 'West Godavari', 'Y.S.R. Kadapa'
    ],
    'Arunachal Pradesh': [
    ...
    'Assam': [
      'Baksa', 'Barpeta', 'Biswanath', 'Bongaigaon', 'Cachar', 'Charaideo', 'Chirang', 'Darrang', 'Dhemaji', 'Dhubri', 'Dibrugarh', 'Dima Hasao', 'Goalpara', 'Golaghat', 'Hailakandi', 'Hojai', 'Jorhat', 'Kamrup', 'Kamrup Metropolitan', 'Karbi Anglong', 'Karimganj', 'Kokrajhar', 'Lakhimpur', 'Majuli', 'Morigaon', 'Nagaon', 'Nalbari', 'Dima Hasao', 'Sivasagar', 'Sonitpur', 'South Salmara-Mankachar', 'Tinsukia', 'Udalguri', 'West Karbi Anglong'
    ],
    'Bihar': [
    ...
    'Chhattisgarh': [
      'Balod', 'Baloda Bazar', 'Balrampur', 'Bastar', 'Bemetara', 'Bijapur', 'Bilaspur', 'Dantewada', 'Dhamtari', 'Durg', 'Gariaband', 'Gaurela-Pendra-Marwahi', 'Janjgir-Champa', 'Jashpur', 'Kanker', 'Kawardha', 'Kondagaon', 'Korba', 'Koriya', 'Mahasamund', 'Mungeli', 'Narayanpur', 'Raigarh', 'Raipur', 'Rajnandgaon', 'Sukma', 'Surajpur', 'Surguja'
    ],
    'Dadra and Nagar Haveli and Daman and Diu': [
    ...
    'Jammu and Kashmir': [
      'Anantnag', 'Bandipora', 'Baramulla', 'Budgam', 'Doda', 'Ganderbal', 'Jammu', 'Kathua', 'Kishtwar', 'Kulgam', 'Kupwara', 'Leh', 'Poonch', 'Pulwama', 'Rajouri', 'Ramban', 'Reasi', 'Samba', 'Shopian', 'Srinagar', 'Udhampur'
    ],
    'Jharkhand': [
    ...
    'Odisha': [
      'Angul', 'Balangir', 'Balasore', 'Bargarh', 'Bhadrak', 'Boudh', 'Cuttack', 'Deogarh', 'Dhenkanal', 'Gajapati', 'Ganjam', 'Jagatsinghapur', 'Jajpur', 'Jharsuguda', 'Kalahandi', 'Kandhamal', 'Kendrapara', 'Kendujhar', 'Khordha', 'Koraput', 'Malkangiri', 'Mayurbhanj', 'Nabarangpur', 'Nayagarh', 'Nuapada', 'Puri', 'Rayagada', 'Sambalpur', 'Subarnapur', 'Sundargarh'
    ],
    'Puducherry': [
      'Karaikal', 'Mahe', 'Puducherry', 'Yanam'
    ],
    'Punjab': [
      'Amritsar', 'Barnala', 'Bathinda', 'Faridkot', 'Fatehgarh Sahib', 'Fazilka', 'Ferozepur', 'Gurdaspur', 'Hoshiarpur', 'Jalandhar', 'Kapurthala', 'Ludhiana', 'Mansa', 'Moga', 'Muktsar', 'Pathankot', 'Patiala', 'Rupnagar', 'Sahibzada Ajit Singh Nagar', 'Sangrur', 'Shaheed Bhagat Singh Nagar', 'Tarn Taran'
    ],
    'Rajasthan': [
  ...
  const districtOptions = districtsByState[selectedState] || [];
      ...
      >
        ...
        <div className='dropdown-container1'>
          <select value={selectedDistrict} onChange={handleDistrictChange}>
            ...
            {selectedState !== '' && districtOptions.map((district, index) => (
        ...
        {customPlaces.map((place, index) => {
          if (
            (selectedState === 'All Sites' || place.state.toLowerCase() === selectedState.toLowerCase()) &&
            (selectedDistrict === '' || place.district.toLowerCase() === selectedDistrict.toLowerCase())
          ) {
            return (
              <Marker
                key={index}
